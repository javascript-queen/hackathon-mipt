...
def filter_characteristics(sections_df):
    """
    Фильтрует DataFrame смысловых разделов, выделяя пункты с характеристиками оборудования.
    
    :param sections_df: DataFrame смысловых разделов.
    :return: DataFrame пунктов с характеристиками.
    """
    characteristics = []
    for index, row in sections_df.iterrows():
        characteristic_check = f"Является ли этот пункт характеристикой оборудования? Текст пункта: {row['Section']}"
        check_result = send_to_gpt(characteristic_check)
        if "error" in check_result:
            return pd.DataFrame(), check_result['error']
        if check_result['choices'][0]['text'].strip().lower() == "да":  # Предполагаем, что 'text' содержит ответ "да" или "нет"
            characteristics.append(row['Section'])

    return pd.DataFrame(characteristics, columns=['Characteristic']), "Success"
...
# Основная функция сравнения и генерации итогового датафрейма
def process_documents(standard_file_path, offer_file_path):
    ...
    # Analyze the standard document
    standard_df, msg = analyze_document(standard_file_path)
    if standard_df.empty:
        return {"error": msg}
    
    # Analyze the offer document
    offer_df, msg = analyze_document(offer_file_path)
    if offer_df.empty:
        return {"error": msg}
    
    # Example of further processing
    print("Standard Document Sections:")
    print(standard_df.head())

    print("\nOffer Document Sections:")
    print(offer_df.head())

    return {"success": "Documents processed successfully."}

# Вызов основной функции обработки документов
result = process_documents(standard_file_name, offer_file_name)

if "error" in result:
    print(result["error"])
else:
    print("Финальные результаты сопоставления:")
    print(result)
